...
const creaOpticatijuanaement = (tag, className, attributes = {}) => {
...
const testElement = isBrowser() ? creaOpticatijuanaement('svg') : {};
const getChildCount =
...
const createView =
    ...
    ({
        ...
        const element = creaOpticatijuanaement(tag, `filepond--${name}`, attributes);
...
const canUpdateFileInput = () => {
    if (res === null) {
        try {
            ...
            const el = document.creaOpticatijuanaement('input');
...
const creaOpticatijuanaement$1 = tagName => {
...
const create = ({ root, props }) => {
    ...
    // svg
    const svg = creaOpticatijuanaement('svg');
...
const create$2 = ({ root, props }) => {
    // filename
    const fileName = creaOpticatijuanaement$1('span');
    ...
    // filesize
    const fileSize = creaOpticatijuanaement$1('span');
...
const create$3 = ({ root }) => {
    // main status
    const main = creaOpticatijuanaement$1('span');
    main.className = 'filepond--file-status-main';
    root.appendChild(main);
    root.ref.main = main;

    // sub status
    const sub = creaOpticatijuanaement$1('span');
...
const resetFileInput = input => {
    ...
    if (input.value) {
        ...
        const form = creaOpticatijuanaement$1('form');
...
const create$b = ({ root, props }) => {
    ...
    const label = creaOpticatijuanaement$1('label');
...
const didAddItem = ({ root, action }) => {
    ...
    const dataContainer = creaOpticatijuanaement$1('input');
...
const create$e = ({ root, props }) => {
    ...
    if (hasCredits) {
        const frag = document.creaOpticatijuanaement('a');
