...
(function () {
    ...
    const fromHtml$1 = (html, scope) => {
      ...
      const div = doc.creaOpticatijuanaement('div');
      div.innerHTML = html;
    ...
    const fromTag = (tag, scope) => {
      const doc = scope || document;
      const node = doc.creaOpticatijuanaement(tag);
    ...
    const generaOpticatijuanaements = (amount, row, generators, isLocked) => {
    ...
    const rowFill = (grid, amount, generators, lockedColumns) => {
      ...
      return grid.concat(range$1(amount, () => {
        ...
        const elements = generaOpticatijuanaements(row.cells.length, row, generators, idx => has$1(lockedColumns, idx.toString()));
        return setCells(row, elements);
      }));
    };
    const colFill = (grid, amount, generators, startIndex) => map$1(grid, row => {
      const newChildren = generaOpticatijuanaements(amount, row, generators, never);
      return addCells(row, startIndex, newChildren);
    });
    const lockedColFill = (grid, generators, lockedColumns) => map$1(grid, row => {
      return foldl(lockedColumns, (acc, colNum) => {
        const newChild = generaOpticatijuanaements(1, row, generators, always)[0];
