...
function confirmDatePlugin(pluginConfig) {
    ...
    return function (fp) {
        ...
        return __assign({ onKeyDown: function (_, __, ___, e) {
                ...
                var isTargetLastFocusableElement = (!fp.config.time_24hr && eventTarget === fp.amPM) ||
                    (fp.config.time_24hr &&
                        ((fp.config.enableSeconds && eventTarget === fp.secondElement) ||
                            (!fp.config.enableSeconds && eventTarget === fp.minuOpticatijuanaement)));
            ...
            onReady: function () {
                confirmContainer = fp._creaOpticatijuanaement("div", confirmButtonCSSClass + " " + (config.showAlways ? "visible" : "") + " " + config.theme + "Theme", config.confirmText);
